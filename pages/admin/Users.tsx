...
const AdminUsers: React.FC = () => {
  ...
  const handleCreateUser = async () => {
    ...
    try {
      ...
      setNewUser({ name: '', email: '', role: UserRole.CUSTOMER, phone: '', locationName: 'Dubai, UAE' });
    } catch (err) {
    ...
    <div className="flex flex-col min-h-screen pb-32 bg-transparent">
      <header className="px-6 pt-10 pb-4">
        ...
        <div className="flex justify-between items-center px-1 mb-4 relative">
          <h3 className="text-[11px] font-bold text-[#888888] uppercase tracking-widest">{activeTab === 'Requests' ? 'Applications' : 'Directory'}</h3>
      ...
      <main className="flex-1 px-4 pb-10">
        <div className="bg-white rounded-3xl shadow-card border border-[#E0E0E0] overflow-hidden">
          <div className="grid grid-cols-[2.5fr_1.2fr_1fr] px-6 py-4 border-b border-[#F0F0F0] bg-gray-50/30">
            <span className="text-[11px] font-bold text-[#888888] uppercase tracking-wider">{activeTab === 'Requests' ? 'BUSINESS' : 'USER'}</span>
            <span className="text-[11px] font-bold text-[#888888] uppercase tracking-wider text-center">{activeTab === 'Requests' ? 'STATUS' : 'ROLE'}</span>
            <span className="text-[11px] font-bold text-[#888888] uppercase tracking-wider text-right">{activeTab === 'Requests' ? 'SUBMITTED' : 'JOINED'}</span>
          </div>
          <div className="divide-y divide-[#F0F0F0]">
            {activeTab === 'Requests' ? (
              (filteredAndSortedItems as ProviderRequest[]).map((req) => {
                ...
                return (
                  <div key={req.id} onClick={() => navigate(`/admin/provider-request/${req.id}`)} className="grid grid-cols-[2.5fr_1.2fr_1fr] items-center px-6 py-5 active:bg-[#F9F9F9] transition-colors cursor-pointer animate-in fade-in duration-300">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="w-12 h-12 rounded-2xl bg-primary/5 flex items-center justify-center text-primary shrink-0"><span className="material-symbols-outlined">storefront</span></div>
                      <div className="min-w-0"><h4 className="text-sm font-bold text-[#333333] truncate uppercase">{req.businessName}</h4><p className="text-[11px] text-[#888888] truncate font-medium">{req.contactPerson}</p></div>
                    </div>
                    <div className="flex justify-center"><span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider ${req.status === 'PENDING' ? 'bg-[#FFF9E5] text-[#FFB100]' : req.status === 'APPROVED' ? 'bg-[#F0F9EB] text-[#8BC34A]' : 'bg-[#FFEBEB] text-red-50'}`}>{req.status}</span></div>
                    <div className="text-right"><p className="text-[11px] font-bold text-[#333333] uppercase">{date}</p><p className="text-[9px] font-bold text-[#888888] uppercase mt-0.5">{time}</p></div>
                  </div>
                );
              })
            ) : (
              (filteredAndSortedItems as User[]).map((user) => {
                ...
                return (
                  <div key={user.id} onClick={() => navigate(`/admin/user/${user.id}`)} className="grid grid-cols-[2.5fr_1.2fr_1fr] items-center px-6 py-5 active:bg-[#F9F9F9] transition-colors cursor-pointer animate-in fade-in duration-300">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="relative shrink-0"><img src={user.avatar} className="w-12 h-12 rounded-full object-cover border border-[#E0E0E0]" alt="" /><div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${user.isBlocked ? 'bg-[#EF4444]' : 'bg-[#22C55E]'}`}></div></div>
                      <div className="min-w-0"><h4 className="text-sm font-bold text-[#333333] truncate uppercase">{user.name}</h4><p className="text-[11px] text-[#888888] truncate font-medium lowercase">{user.email}</p></div>
                    </div>
                    <div className="flex justify-center"><span className={`px-3 py-1 rounded-lg text-[10px] font-bold inline-block min-w-[70px] text-center ${user.isBlocked ? 'bg-[#FEE2E2] text-[#EF4444]' : user.role === UserRole.PROVIDER ? 'bg-[#EBE7F5] text-primary' : 'bg-[#F1F5F9] text-[#64748B]'}`}>{user.isBlocked ? 'Blocked' : user.role.charAt(0) + user.role.slice(1).toLowerCase()}</span></div>
                    <div className="text-right"><p className="text-[11px] font-bold text-[#333333] uppercase whitespace-nowrap">{date}</p><p className="text-[9px] font-bold text-[#888888] uppercase mt-0.5">{time}</p></div>
                  </div>
                );
              })
            )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl p-8 space-y-6 animate-in zoom-in-95 duration-200">
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-text-dark tracking-tight">Manual Onboarding</h3>
            </div>
            <div className="space-y-4">
              ...
              <input type="text" placeholder="Full Name" className="w-full px-5 py-3.5 bg-gray-50 border-none rounded-2xl text-xs shadow-inner" value={newUser.name} onChange={e => setNewUser({...newUser, name: e.target.value})} />
              <input type="email" placeholder="Email Address" className="w-full px-5 py-3.5 bg-gray-50 border-none rounded-2xl text-xs shadow-inner" value={newUser.email} onChange={e => setNewUser({...newUser, email: e.target.value})} />
            </div>
            <div className="flex gap-3 pt-2">
              <button onClick={() => setIsCreateModalOpen(false)} className="flex-1 py-4 text-xs font-bold text-gray-400 bg-gray-50 rounded-2xl">Cancel</button>
              <button onClick={handleCreateUser} disabled={isCreating} className="flex-1 py-4 bg-primary text-white text-xs font-bold rounded-2xl shadow-xl">
