  ...
  return (
    <div className="flex flex-col min-h-screen bg-transparent pb-12">
      <header className="px-4 pt-10 pb-4 flex items-center justify-between sticky top-0 bg-white/20 backdrop-blur-md z-50">
        ...
        <h1 className="text-[17px] font-bold text-text-dark text-center flex-1 uppercase tracking-tight">Customer Integrity Record</h1>
        ...
        <section className="space-y-4">
           ...
           <div className="bg-white rounded-[2.2rem] shadow-sm border border-white p-6 space-y-0">
              {activityLog.length > 0 ? activityLog.map((log, idx) => (
                <div 
                  key={idx} 
                  onClick={() => log.rfqId && navigate(`/rfq/${log.rfqId}`)}
                  className={`flex gap-4 relative pb-8 group last:pb-2 ${log.rfqId ? 'cursor-pointer active:opacity-60' : ''}`}
                >
                   ...
                   <div className="flex-1 min-w-0">
                      ...
                      <div className="flex justify-between items-center mt-1">
                         <p className="text-[10px] text-gray-300 font-bold uppercase">{log.dateStr}</p>
                         {log.rfqId && <span className="text-[9px] font-black text-primary uppercase tracking-widest flex items-center">View Query <span className="material-symbols-outlined text-xs">chevron_right</span></span>}
                      </div>
                   </div>
                </div>
              )) : (
