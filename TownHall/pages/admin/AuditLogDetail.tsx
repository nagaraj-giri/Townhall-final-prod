  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      const unsub = dataService.listenToAuditLogs((logs) => {
        ...
        if (matched) {
          setLog(matched);
          // Fix: Accessed userId directly from flat log structure to resolve property error
          if (matched.userId) {
            dataService.getUserById(matched.userId).then(setActor);
          } else if (matched.actor?.userId) {
            dataService.getUserById(matched.actor.userId).then(setActor);
          }
        }
        setLoading(false);
