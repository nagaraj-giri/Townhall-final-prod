  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FAF9F6] pb-10">
      <header className="px-4 pt-12 pb-4 flex items-center justify-between sticky top-0 bg-white/20 backdrop-blur-md z-50">
        ...
        <h1 className="text-[17px] font-bold text-text-dark text-center flex-1 uppercase tracking-tight">Provider Intelligence Console</h1>
      ...
      <main className="px-6 space-y-8 overflow-y-auto no-scrollbar pt-2 pb-20">
        ...
        <section className="space-y-4">
           <div className="flex items-center justify-between px-1">
              <h3 className="text-[13px] font-black text-gray-400 uppercase tracking-widest">Market Engagement</h3>
           </div>
           <div className="grid grid-cols-3 gap-3">
              <div className="bg-white p-5 rounded-[1.8rem] shadow-sm border border-white text-center">
                 <p className="text-xl font-black text-primary">{stats.quotesSent}</p>
                 <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">Bids Sent</p>
              </div>
              <div className="bg-white p-5 rounded-[1.8rem] shadow-sm border border-white text-center">
                 <p className="text-xl font-black text-accent-green">AED {stats.sales}</p>
                 <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">Hiring Value</p>
              </div>
              <div className="bg-white p-5 rounded-[1.8rem] shadow-sm border border-white text-center">
                 <p className="text-xl font-black text-secondary">{stats.rating}</p>
                 <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mt-1">Avg Rating</p>
              </div>
           </div>
        </section>
        ...
        <section className="space-y-4">
           <h3 className="text-[13px] font-black text-gray-400 ml-1 uppercase tracking-widest">Recent Bid Activity</h3>
           <div className="bg-white rounded-[2.5rem] shadow-card border border-white overflow-hidden divide-y divide-gray-50">
              {providerQuotes.slice(0, 5).map(q => (
                <div key={q.id} onClick={() => navigate(`/rfq/${q.rfqId}`)} className="p-5 flex items-center justify-between active:bg-gray-50 cursor-pointer transition-colors group">
                   <div className="flex-1 min-w-0 pr-4">
                      <p className="text-[10px] font-black text-primary uppercase tracking-tighter mb-1">Bid ID: #{q.id.slice(-6).toUpperCase()}</p>
                      <h4 className="text-[13px] font-bold text-text-dark truncate">AED {q.price}</h4>
                      <p className="text-[9px] text-gray-400 font-medium mt-1 truncate">{q.message}</p>
                   </div>
                   <div className="flex flex-col items-end shrink-0 gap-1.5">
                      <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider ${q.status === 'ACCEPTED' ? 'bg-accent-green/10 text-accent-green' : 'bg-gray-100 text-gray-400'}`}>
                        {q.status}
                      </span>
                      <p className="text-[9px] font-bold text-gray-300">{new Date(q.createdAt).toLocaleDateString()}</p>
                   </div>
                </div>
              ))}
              {providerQuotes.length === 0 && (
                <div className="py-12 text-center opacity-30">
                   <p className="text-[10px] font-black uppercase tracking-widest">No active bids logged</p>
                </div>
              )}
           </div>
        </section>
           ...
           <div className="bg-white rounded-[2.5rem] shadow-card border border-white p-6">
              <div className="grid grid-cols-3 gap-3">
                 {editForm.gallery.map((img, idx) => (
                   <div key={idx} className="relative aspect-square rounded-2xl overflow-hidden shadow-sm border border-gray-100 bg-gray-50 group">
