...
const Profile: React.FC<ProfileProps> = ({ user, onLogout, onUpdateUser }) => {
  ...
  const handleRemoveGalleryPhoto = async (url: string) => {
    ...
    setGallery(newGallery);
    try {
      const updatedUser: User = { ...user, gallery: newGallery };
      await dataService.saveUser(updatedUser);
      if (onUpdateUser) onUpdateUser(updatedUser);
        ...
        <div className="flex items-center gap-5">
          <div className="relative group cursor-pointer" onClick={handleAvatarClick}>
            <div className="w-24 h-24 rounded-[2rem] border-[3px] border-white shadow-soft overflow-hidden bg-white p-1">
              <img src={user.avatar} className={`w-full h-full object-cover rounded-[1.8rem] ${isUploadingAvatar ? 'opacity-50' : ''}`} alt="" />
          ...
          <div className="flex-1 min-w-0 pr-12 relative">
            <div onClick={() => navigate(`/storefront/${user.id}`)} className="cursor-pointer group">
              <h2 className="text-[20px] font-black text-text-dark leading-tight uppercase truncate group-hover:text-primary transition-colors">
                {form.companyName || 'New Business'}
              </h2>
              <p className="text-[10px] text-gray-400 font-bold uppercase tracking-tight mt-0.5 group-hover:text-primary/70">
                {form.description ? (form.description.length > 30 ? form.description.substring(0, 30) + '...' : form.description) : 'Verified Service Provider'}
              </p>
            </div>
        ...
        <section className="space-y-4">
           ...
           <div className="bg-white rounded-[2.5rem] p-8 shadow-soft border border-white space-y-6">
              {[
                ...
                { label: 'About Us', value: form.description, icon: 'info', key: 'description' },
                      ...
                      <div className="flex-1 min-w-0 flex items-center gap-2">
                         ...
                         {isEditing ? (
                           field.isLocation ? (
                             ...
                             <textarea 
                               className="w-full bg-transparent border-none p-0 text-[14px] font-bold text-text-dark focus:ring-0 outline-none resize-none min-h-[80px]"
                               value={field.value}
                               placeholder="Add a about us field"
                               onChange={(e) => setForm({...form, [field.key]: e.target.value})}
                             />
                           ) : (
                             <div className="flex items-center gap-2 w-full">
                               {field.isPhone && (
                                 <div className="flex items-center gap-1.5 border-r border-gray-200 pr-3 shrink-0">
                           ...
                           <p className="text-[14px] font-bold text-text-dark truncate">
                             {field.isPhone && field.value ? field.value : (field.value || 'Not Set')}
                           </p>
                         )}
                      </div>
                   </div>
