  ...
  const currentList = useMemo(() => {
    ...
    if (activeTab === 'Active') {
      return allRfqs.filter(r => {
        // Exclude statuses that are no longer available for bidding
        if (['CANCELED', 'COMPLETED', 'ACCEPTED'].includes(r.status)) return false;
        
        // Exclude if provider already submitted a proposal
        if (myQuoteRfqIds.includes(r.id)) return false;

        // Match service expertise
        const isCategoryMatch = (user.services || []).includes(r.service);
        if (!isCategoryMatch) return false;

        // Verify Spatial Match (Fixing the empty board by ensuring location object path is correct)
    ...
    if (activeTab === 'Pending') {
      ...
      return allRfqs.filter(r => pendingRfqIds.includes(r.id));
    }
    
    if (activeTab === 'Accepted') {
      const acceptedRfqIds = myQuotes.filter(q => q.status === 'ACCEPTED').map(q => q.rfqId);
      return allRfqs.filter(r => acceptedRfqIds.includes(r.id));
    }
    ...
    <div className="flex flex-col min-h-screen pb-28 bg-transparent">
      <header className="px-6 pt-12 pb-4 flex items-center justify-between sticky top-0 bg-white/10 backdrop-blur-md z-40">
        ...
        <button onClick={() => toggleNotifications(true)} className="relative w-11 h-11 bg-white rounded-2xl shadow-sm flex items-center justify-center border border-gray-100 active:scale-95 transition-transform">
          ...
          {unreadCount > 0 && <div className="absolute top-2.5 right-2.5 w-2.5 h-2.5 bg-accent-pink rounded-full border-2 border-white shadow-sm animate-pulse"></div>}
        </button>
      </header>
      ...
      <main className="flex-1 px-6 space-y-6 overflow-y-auto no-scrollbar pt-2">
        <div className="bg-[#FFFBE6] border border-[#FFF1B8] p-4 rounded-[1.5rem] flex items-center gap-3">
          ...
          <p className="text-[9px] font-normal text-text-dark leading-tight uppercase tracking-wider">
        ...
        <div className="flex gap-1 justify-between bg-white rounded-full p-1 border border-gray-100 shadow-soft">
          {(['Active', 'Pending', 'Accepted', 'Completed'] as ProviderTab[]).map((tab) => (
            <button
              ...
              className={`flex-1 py-3.5 rounded-full text-[10px] font-normal uppercase tracking-widest transition-all ${
                activeTab === tab
        ...
        <div className="space-y-5">
          {isLoading ? (
            ...
            currentList.map((rfq) => {
              ...
              const pLat = Number(user.location?.lat || 25.185);
              const pLng = Number(user.location?.lng || 55.275);
              const rLat = Number(rfq.location?.lat);
              const rLng = Number(rfq.location?.lng);
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center text-primary shadow-sm border border-primary/5">
                        <span className="material-symbols-outlined text-2xl font-normal">assignment</span>
                      </div>
                      <div>
                        <h2 className="text-sm font-black text-text-dark leading-tight uppercase truncate max-w-[160px]">{rfq.title}</h2>
                        <p className="text-[10px] text-gray-400 font-normal mt-1 uppercase tracking-tight">{rfq.category} • {displayLoc}</p>
                      </div>
                    </div>
                    <div className={`px-4 py-1.5 rounded-xl text-[9px] font-normal uppercase tracking-wider ${
                  ...
                  <div className="grid grid-cols-2 gap-4 py-4 border-b border-gray-50 mb-4">
                    <div>
                      <p className="text-[9px] font-normal text-gray-300 uppercase tracking-widest">Bid Amount</p>
                      <p className="text-lg font-normal text-primary">{myQuote ? `AED ${myQuote.price}` : '...'}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[9px] font-normal text-gray-300 uppercase tracking-widest">Query ID</p>
                      <p className="text-base font-normal text-text-dark">{rfq.idDisplay}</p>
                    </div>
                  </div>

                  <div className="flex justify-between items-center">
                    <div className="text-[10px] text-gray-300 font-normal uppercase tracking-tight">
                      {new Date(rfq.createdAt).toLocaleDateString()}
                    </div>
                    <div className="flex items-center gap-1 text-primary text-[11px] font-normal uppercase tracking-widest">
                      Manage
                      <span className="material-symbols-outlined text-sm font-normal">chevron_right</span>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="py-32 text-center flex flex-col items-center justify-center">
               ...
               <p className="text-[12px] font-normal text-gray-300 uppercase tracking-[0.3em] text-center leading-relaxed max-w-[200px]">
