...
// Brand Approved Working Logo URL
const LOGO_URL = "https://i.postimg.cc/mD8z7DqZ/townhall-logo.png";

export const EmailDispatcher = {
  
  sendBulkPhasedEmail: async (recipientEmails: string[], data: any, rfqId: string) => {
    try {
      ...
      const finalSubject = override?.subject || template.data.Subject;
      const finalHtmlBody = override?.html || template.data.html;

      await dataService.sendTemplatedEmail({
        to: recipientEmails,
        metadata: { rfqId, batchType: "AIRRA_PHASED_MATCH", triggeredAt: new Date().toISOString() },
        message: {
          ...
          html: `
            <div style="font-family: sans-serif; padding: 40px 20px; color: #333; max-width: 550px; margin: 0 auto; border-radius: 40px; background-color: #ffffff; border: 1px solid #eee;">
              <div style="text-align: center; margin-bottom: 40px;">
                <img src="${LOGO_URL}" width="100" style="display: inline-block; border: 0;" alt="Town Hall UAE">
              </div>
  ...
  sendTriggerEmail: async (recipientEmail: string, templateKey: keyof typeof EMAIL_TEMPLATES, data: any) => {
    try {
      const config = await dataService.getEmailConfig();
      if (config.triggers && config.triggers[templateKey]?.email === false) return;

      const templateGenerator = EMAIL_TEMPLATES[templateKey];
      // @ts-ignore
      const template = templateGenerator(data);
      ...
      await dataService.sendTemplatedEmail({
        to: [recipientEmail],
        metadata: { templateKey, triggeredAt: new Date().toISOString() },
        message: {
          subject: finalSubject,
          html: `
            <div style="font-family: sans-serif; padding: 40px 20px; color: #333; max-width: 550px; margin: 0 auto; border-radius: 40px; background-color: #ffffff; border: 1px solid #eee;">
              ...
              <div style="padding: 0 25px;">
                ${finalHtmlBody}
                ${template.data.buttonUrl ? `
                <div style="margin-top: 40px; text-align: center;">
                  <a href="${template.data.buttonUrl}" style="background-color: #5B3D9D; color: #ffffff; padding: 18px 40px; text-decoration: none; border-radius: 25px; font-weight: bold; font-size: 14px; display: inline-block;">
                    ${template.data.buttonLabel || 'View on Platform'}
                  </a>
                </div>
                ` : ''}
              </div>
              <div style="margin-top: 60px; border-top: 1px solid #f5f5f5; padding-top: 30px; text-align: center; color: #ccc; font-size: 10px; text-transform: uppercase; letter-spacing: 1px;">
                Town Hall UAE • Premium UAE Marketplace
              </div>
            </div>
          `
        }
      ...
      console.error(`[EmailEngine] Trigger dispatch failed:`, error);
    }
  }
};